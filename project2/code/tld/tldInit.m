...
function tld = tldInit(opt,tld)
    ...
    % tld.source.idx -> Number of frames
    ...
    [bbP, patches] = tldGeneratePositiveData(tld,overlap,tld.img{1},tld.p_par_init);
    
    % Select closest bbox (Adjusted to the grid)
    [~, mxo] = max(overlap);
    tld.bb(:,1) = tld.grid(1:4, mxo);
    
    best_patch = patches{1,1}; % This should be the best patch
    tld.var = var(best_patch(:));
    % tld.bb(:,1) -> First frame best bbox. 
    ...
    patch_variances = zeros(size(patches));
    for p=1:size(patches,1)
        for w=1:size(patches,2)
            this_patch = patches{p,w};
            patch_variances(p,w) = var(this_patch(:));
        end
    end
    prod(size(patch_variances))
    size(patch_variances(patch_variances > (tld.var/2)))
    tld.var
    pause
    %% ------------------------- (END) ----------------------------------------
